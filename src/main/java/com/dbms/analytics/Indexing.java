...
public class Indexing {
    ...
    private static final Map<String, List<Index>> CLUSTERING_CONFIG = Map.ofEntries(
            entry(SAILORS, Arrays.asList(new Index(SailorsA, ORDER, true))),
            entry(BOATS, Arrays.asList(new Index(BoatsD, ORDER, true))));
    ...
    /** Runs benchmarking task for all 3 join types on randomly generated data. Each trial generates
     * a new random data set and runs the 3 queries using each join type. If no argument is
     * provided, it runs 1 trial by default.
     *
